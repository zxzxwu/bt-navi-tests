...
GENERIC_VOICE_ASSISTANT_SERVICE_UUID = core.UUID.from_16_bits(
    0x185F, 'Generic Voice Assistant Service'
)
VOICE_ASSISTANT_SERVICE_UUID = core.UUID.from_16_bits(
    0x185E, 'Voice Assistant Service'
)
VA_NAME_CHARACTERISTIC_UUID = core.UUID.from_16_bits(
...
VA_SESSION_FLAG_CHARACTERISTIC_UUID = core.UUID.from_16_bits(
    0x2C36, 'Voice Assistant Session Flag'
)
VA_SESSION_STATE_CHARACTERISTIC_UUID = core.UUID.from_16_bits(
...
VA_UUID_CHARACTERISTIC_UUID = core.UUID.from_16_bits(
...
class VoiceAssistantService(gatt.TemplateService):
  ...
  def __init__(
    ...
    self.va_name = gatt_adapters.UTF8CharacteristicAdapter(
        gatt.Characteristic[str](
    ...
    self.va_supported_features = gatt_adapters.EnumCharacteristicAdapter[
        VaSupportedFeatures
    ](
  ...
  async def _on_vas_control_point_write(
    ...
    await connection.device.notify_subscriber(
        connection, self.vas_control_point, value=response
    )
...
class VoiceAssistantServiceProxy(gatt_client.ProfileServiceProxy):
  ...
  def __init__(self, service_proxy: gatt_client.ServiceProxy) -> None:
    ...
    self.va_session_state_proxy = (
        service_proxy.get_required_characteristic_by_uuid(
            VA_SESSION_STATE_CHARACTERISTIC_UUID
        )
    )
  ...
  async def subscribe_characteristics(self) -> None:
    ...
    await self.vas_control_point.subscribe(
        self.vas_control_point_notifications.put_nowait
    )
  ...
  async def write_control_point(
      self, opcode: ControlPointOpcode
  ) -> ResponseCodeValue:
  ...
  async def initialize_va_session(self) -> ResponseCodeValue:
    return await self.write_control_point(
        ControlPointOpcode.INITIALIZE_VA_SESSION
    )
