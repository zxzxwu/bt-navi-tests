...
class VolumeOffsetControlService(gatt.TemplateService):
    ...
    self.location_characteristic = gatt_adapters.EnumCharacteristicAdapter(
        gatt.Characteristic(
            ...
            value=gatt.CharacteristicValue(
                read=self._read_audio_location_value
            ),
        ),
  ...
  async def on_description_write(
      self, connection: device.Connection, value: str
  ) -> None:
    self.audio_output_description = value
    await self.on_location_or_description_write(connection, value)

  async def on_location_or_description_write(
      self, connection: device.Connection, _: Any
  ) -> None:
    self.change_counter = (self.change_counter + 1) % 256
    await connection.device.notify_subscribers(
        self.state_characteristic, bytes(self)
    )

  async def on_control_point_write(
    ...
    if opcode == VocsControlPointOpcode.SET_VOLUME_OFFSET:
      ...
      await connection.device.notify_subscribers(
          self.control_point_characteristic, bytes([0x01, 0x01])
      )
    else:
      await connection.device.notify_subscribers(
          self.control_point_characteristic,
...
class VolumeOffsetControlServiceProxy(gatt_client.ProfileServiceProxy):
  ...
  async def subscribe_characteristics(self) -> None:
    ...
    await self.control_point.subscribe(
        self.vocs_control_point_notifications.put_nowait
    )
    self.state = await gatt_helper.MutableCharacteristicState.create(
        self.state_proxy
    )

  async def write_control_point(
      self, opcode: VocsControlPointOpcode
  ) -> ResponseCodeValue:
  ...
  async def set_volume_offset(self, volume_offset: int) -> None:
    await self.control_point.write_value(
        bytes([VocsControlPointOpcode.SET_VOLUME_OFFSET])
        + struct.pack('<h', volume_offset),
    )
